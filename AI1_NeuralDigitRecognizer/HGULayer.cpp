...
void HGULayer::InitializeWeight()
// Xavier initialization
{
	float stdev = (float)sqrt(2.F / (m_inputDim + m_outputDim));

	unsigned seed = (unsigned)std::chrono::system_clock::now().time_since_epoch().count();
	std::default_random_engine generator(seed);
	std::normal_distribution<float> dist(0.F, stdev);

	float *wLimit = m_aWeight + m_weightDim;
	int i = 0;						// input index
	for (float *pw = m_aWeight; pw < wLimit; pw++, i++){
		if (i == m_inputDim){		// bias
			*pw = 0.F;
			i = 0;
		} else {					// connection weight
			*pw = (float)dist(generator);
		}
	}
}

int HGULayer::Load(FILE *fp)
{
	if (m_inputDim <= 0){
		printf("Layer was not allocated! in %s (%s %d)\n", __FUNCTION__, __FILE__, __LINE__);
		return FALSE;
	}

	if (fp == NULL){
		printf("Failed to load layer, fp == NULL in %s (%s %d)\n", __FUNCTION__, __FILE__, __LINE__);
		return FALSE;
	}

	int ret = fread(m_aWeight, sizeof(float), m_weightDim, fp);

	if (ret < m_weightDim){
		printf("Failed to load layer. m_weightDim = %d, ret = %d in %s (%s %d)\n", m_weightDim, ret, __FUNCTION__, __FILE__, __LINE__);
		return FALSE;
	}
...
int HGULayer::Save(FILE *fp)
{
	...
	if (fp == NULL){
		printf("Failed to save layer, fp == NULL in %s (%s %d)\n", __FUNCTION__, __FILE__, __LINE__);
		return FALSE;
	}

	int ret = fwrite(m_aWeight, sizeof(float), m_weightDim, fp);

	if (ret < m_weightDim){
		printf("Failed to save weights. m_weightDim = %d, ret = %d in %s (%s %d)\n", m_weightDim, ret, __FUNCTION__, __FILE__, __LINE__);
		return FALSE;
	}
